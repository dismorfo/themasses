...
function dlts_book_preprocess_page(&$vars) {
  ...
  $site_environment = theme_get_setting('dlts_book_environment');
  ...
  drupal_add_js('http://yui.yahooapis.com/3.9.0pr1/build/yui/yui-min.js', 'external', array('group' => JS_LIBRARY, 'weight' => -100 ));
  ...
  if (isset($vars['node'])) {
    ...
    switch ( $vars['node']->type ) {
      ...
      case 'dlts_book_stitched_page' :
        ...
        /** Remove tabs from book page and stitched page */      
        $vars['tabs'] = theme_get_setting('dlts_book_toggle_page_tabs') ? $vars['tabs'] : NULL;
        break;
        
      case 'page' :
      
        if (drupal_is_front_page() && 1 == 2) {
          ...
          /** Load the first 10 books of the collection */
          $books = module_invoke('views', 'block_view', 'books_json-block_1');

          /** Add blocks to node */
          $vars['node']->books = $books;
          $vars['node']->search_box = $search;

          /** Add search information to DLTS (JS) namespace */
          $js_data += array(
              'books' => array(
                  'templates' => array(
                      'sequence' => theme_mustache_books_sequence(),
                      'container' => theme_mustache_container_sequence(),
                  ),
              ),
          );

          /** Add all the necesary JS files */
          drupal_add_js($js_data, 'setting');
          drupal_add_js($theme_path . '/js/ui.datasource.yui.js', array('type' => 'file', 'scope' => 'footer', 'weight' => 5));
          drupal_add_js($theme_path . '/js/ui.books.view.js', array('type' => 'file', 'scope' => 'footer', 'weight' => 6));
...
function dlts_book_preprocess_node(&$vars) {
  ...
  switch ($vars['type']) {
    ...
    case 'dlts_book_stitched_page' :
      ...
      /** Use node--dlts-book-page.tpl.php for both dlts_book_page and dlts_book_stitched_page contetn types */
      if (dlts_utilities_is_pjax()) {
        $vars['theme_hook_suggestions'][] = 'node__dlts_book_pjax_page';
        return;
      }
      
      else {
        $vars['theme_hook_suggestions'][] = 'node__dlts_book_page';
      }

      /** Add UI YUI */
      drupal_add_js($theme_path . '/js/ui.datasource.yui.js', array('type' => 'file', 'scope' => 'footer', 'weight' => 5));
      drupal_add_js($theme_path . '/js/ui.keyboard.yui.js', array('type' => 'file', 'scope' => 'footer', 'weight' => 5));
      drupal_add_js($theme_path . '/js/ui.components.yui.js', array('type' => 'file', 'scope' => 'footer', 'weight' => 6));
      ...
      $js_data = array(
        'book' => array(
          'path' => url('books/' . $identifier, array('absolute' => TRUE )),
          'theme_path' => $absolute_theme_path,
...
function dlts_book_preprocess_search_result(&$variables) {
  ...
  if ( isset( $variables['result']['fields']['bundle'] ) ) {
    if (preg_match("/_interview/i", $variables['result']['fields']['bundle'])) {
      $variables['oembed'] = $variables['result']['fields']['url'] . '?oembed=true';
    }
  }
